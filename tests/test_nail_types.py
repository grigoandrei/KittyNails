...
async def test_create_nail_type(client):
    response = await client.post("/api/admin/nail-types", json={
        "name": "Short",
        "duration_minutes": 60,
        "price": 30.00,
    })
...
async def test_update_nail_type(client):
    create_resp = await client.post("/api/admin/nail-types", json={
        "name": "Regular",
        "duration_minutes": 75,
        "price": 40.00,
    })
    ...
    update_resp = await client.put(f"/api/admin/nail-types/{nail_type_id}", json={
        "price": 45.00,
    })
...
async def test_update_nonexistent_nail_type(client):
    ...
    response = await client.put(f"/api/admin/nail-types/{fake_id}", json={
        "price": 50.00,
    })
...
async def test_get_active_nail_types_excludes_inactive(client):
    ...
    nail_type_id = create_resp.json()["id"]
    await client.put(f"/api/admin/nail-types/{nail_type_id}", json={
        "is_active": False,
    })
...
async def test_get_active_nail_types_ordered_by_sort_order(client):
    await client.post("/api/admin/nail-types", json={
        "name": "Third", "duration_minutes": 60, "price": 30.00, "sort_order": 3,
    })
    await client.post("/api/admin/nail-types", json={
        "name": "First", "duration_minutes": 60, "price": 30.00, "sort_order": 1,
    })
    await client.post("/api/admin/nail-types", json={
        "name": "Second", "duration_minutes": 60, "price": 30.00, "sort_order": 2,
    })
