...
def update_weekly_availability(schedule: list[WeeklyAvailabilityCreate], db: Session=Depends(get_db), _owner: str = Depends(require_owner)):
    return set_weekly_availability(db, schedule)

@router.post("/block", response_model=BlockedTimeResponse, status_code=201)
def create_blocked_time(block: BlockedTimeCreate, db: Session = Depends(get_db), _owner: str = Depends(require_owner)):
