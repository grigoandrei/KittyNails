...
def create_services(service: NailServiceCreate, db: Session = Depends(get_db)):
    return create_service(db, service=service)

@router.put("/{service_id}", response_model=NailServiceResponse)
def update_services(service_id: int, service: NailServiceUpdate, db: Session = Depends(get_db)):
